...
const receiptPath = path.join(repoRoot, 'plugins', 'huashu-skills', 'release-receipt.json');
...
const versionPaths = [
  ...
  ['plugins/huashu-skills/plugin.json', (value) => value.version],
  ['plugins/huashu-skills/.claude-plugin/plugin.json', (value) => value.version],
  ['plugins/huashu-skills/.codex-plugin/plugin.json', (value) => value.version],
];
...
const manifestPaths = [
  ...
  'plugins/huashu-skills/.codex-plugin/openai.yaml',
...
function payloadDigest() {
  const files = [...manifestPaths, ...collectFiles('plugins/huashu-skills/skills')].sort();
...
function readReceipt() {
  ...
  return readJson('plugins/huashu-skills/release-receipt.json');
}
...
if (write) {
  if (receipt?.version === version && receipt.sha256 !== payload.sha256) {
